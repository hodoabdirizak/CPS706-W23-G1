...
offline_routers_label = tk.Label(window, text = 'List of Offline Routers', font=('calibre', 10, 'bold'))
...
def validate():
    ...
    if not 3 < num_routers < 15:
        canvas.create_text(200, 50, text="Change num_routers to a value between 3 and 15", fill="red", font=('calibre 10 bold'))
    
    elif source_router > num_routers or source_router < 1 or dest_router > num_routers or dest_router < 1:
        canvas.create_text(200, 50, text="Change source_router and dest_router to a value between 1 and {}".format(num_routers), fill="red", font=('calibre 10 bold'))

    elif not re.search("^[-,0-9]+$",offline_routers_raw) or not offline_routers_raw == "":
        canvas.create_text(200, 50, text="offline_routers must follow format: 1,2,3", fill="red", font=('calibre 10 bold')) 
    ...
    if offline_routers_raw == "":
        ...
        canvas.create_text(200, 50, text="Change source router or remove source router from list", fill="red", font=('calibre 10 bold'))    

    elif dest_router in offline_routers:
        canvas.create_text(200, 50, text="Change dest router or remove dest router from list", fill="red", font=('calibre 10 bold'))    
    
    # other errors im prob missing
...
def edges():
    ...
    # create canvas for displaying error msgs
    canvas = tk.Canvas(window, width= 300, height= 150, bg="White")
    canvas.create_text(200, 50, text="edges fxn works", fill="red", font=('calibre 10 bold'))  
    canvas.pack()
